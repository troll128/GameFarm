...
public class AdminMemberUpdateAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		System.out.println("AdminMemberUpdateAction execute()");
		// 한글 출력이 가능하도록!
		request.setCharacterEncoding("utf-8");
		// member.MemberDB에서 updateMember() 메소드를 불러오기 위해 member.MemberDB 클래스 객체를
		// 가져온다.
		...
		if (check == 1) { // 아이디와 비밀번호가 일치할 때
			...
			// 받아온 id와 pass 값과 DB 상의 id와 pass값이 일치하면 member.MemberDB 자바빈의
			// updateMember() 메소드를 실행한다.
			// MemberBean 자바빈에 값을 입력할 변수들을 가져옴
