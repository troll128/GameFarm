...
<body>
<%
// CommunityList에서 리퀘스트 값을 받아온다.
int count = (Integer)request.getAttribute("count");
String pageNum = (String)request.getAttribute("pageNum");
List getCommunityList = (List)request.getAttribute("getCommunityList");
	...
	<div class="container">
					...
					aria-labelledby="headingOne">
						<div class="row">
							<div class="col-lg-6">
								...
								/* List<HotkeyBean> hotkeyList = null;								
								hotkeyList = cdao.hotKeyword(minhotkey, hotkeyCount); */
							...
							<div class="col-lg-6">
								...
								<%
								/* hotkeyList = cdao.hotKeyword(minhotkey, hotkeyCount); */
								if (hotkeyCount>=12) {	// 검색어 테이블에 값이 존재하면?for (int hot=3; hot<6; hot++) {	// 검색어 순위 1위부터 3위까지 반복한다.
			...
			<table id="notice">
				...
				// List getCoList = null; 
					if (count!=0) { // 게시물이 하나 이상 존재할 때						
						for (int c=0; c<getCommunityList.size(); c++) {	// 가져온 리스트의 갯수만큼 행을 반복하여 늘려준다.
							CommunityBean cb = (CommunityBean)getCommunityList.get(c);	// 리스트에 담긴 회원 정보의 값들을 가져온다.							
							%>
							<tr>
		...
		if (pageCount==1) { 
			...
			if (startPage > pageBlock) {	// (시작 페이지가 한 화면당 페이지 수보다 클 때)	
				...
				%>
			<a href="./CommunityList.co?pageNum=<%=startPage-pageBlock%>"></a>
			<li class="page-item"><a class="page-link" href="./CommunityList.co?pageNum=<%=startPage-pageBlock%>"
			...
			for (int p = startPage; p<=endPage; p++) {	// 페이지의 최대 갯수를 계산해주고 그 값을 최대 페이지로 지정하여 for문을 생성	
				...
				%>
					<li class="page-item"><a href="./CommunityList.co?pageNum=<%=p%>" class="page-link"><%=p%></a></li>				
				<%
				}
			}
			// [다음] 버튼을 표시하기
			...
			// 현재 페이지 번호 = 첫 페이지 - 한 화면에 보여줄 페이지 수의 값을 가진 페이지로 이동
			%>
			<li class="page-item"><a class="page-link" href="./CommunityList.co?pageNum=<%=startPage+pageBlock%>"aria-label="Next"> 
		...
		<div class="input-group">
			...
			<span class="input-group-btn"><button class="btn btn-secondary" type="button" onclick="searchCheck()">검색</button></span>
		</div>
		</form>		
		<!-- 검색 파트 -->
	</div>	
