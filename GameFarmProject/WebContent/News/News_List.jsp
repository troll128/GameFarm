	...
	<div class="container">
		...
		<!-- 게시판 -->
			...
			<table id="notice">
				...
				// List getNewsList = null; 
				System.out.println(getNewsList.size());
					if (count!=0) { // 게시물이 하나 이상 존재할 때						
						for (int n=0; n<getNewsList.size(); n++) {	// 가져온 리스트의 갯수만큼 행을 반복하여 늘려준다.
							NewsBean nb = (NewsBean)getNewsList.get(n);	// 리스트에 담긴 회원 정보의 값들을 가져온다.
							System.out.println("게시물"+nb.getNews_subject());
							%>
		...
		if (pageCount==1) { 
			...
			if (startPage > pageBlock) {	// (시작 페이지가 한 화면당 페이지 수보다 클 때)	
				...
				%>
			<a href="./NewsList.nw?pageNum=<%=startPage-pageBlock%>"></a>
			<li class="page-item"><a class="page-link" href="./NewsList.nw?pageNum=<%=startPage-pageBlock%>"
			...
			for (int p = startPage; p<=endPage; p++) {	// 페이지의 최대 갯수를 계산해주고 그 값을 최대 페이지로 지정하여 for문을 생성	
				...
				%>
					<li class="page-item"><a href="./NewsList.nw?pageNum=<%=p%>" class="page-link"><%=p%></a></li>				
				<%
				}
			}
			// [다음] 버튼을 표시하기
			...
			// 현재 페이지 번호 = 첫 페이지 - 한 화면에 보여줄 페이지 수의 값을 가진 페이지로 이동
			%>
			<li class="page-item"><a class="page-link" href="./NewsList.nw?pageNum=<%=startPage+pageBlock%>" aria-label="Next"> 
