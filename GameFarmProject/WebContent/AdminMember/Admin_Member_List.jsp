...
List getMemberList = (List)request.getAttribute("getMemberList");
...
//저장된 정보 가져오기
//request.setAttribute("getmMemberList", getmMemberList);
List getmMemberList=(List)request.getAttribute("getmMemberList");
%>
<!-- 헤더 -->
		...
		<!-- 회원정보 -->
			...
			<table id="notice">
				...
				for(int i=0;i<getMemberList.size();i++){
					MemberBean mb = (MemberBean)getMemberList.get(i);%>
				<tr>
					<td><%=mb.getMem_id() %></td>
					<td class="left">																
	...
	<!-- 페이지 넘버링 -->
		...
		if (pageCount==1) { 
			...
			if (startPage > pageBlock) {	// (시작 페이지가 한 화면당 페이지 수보다 클 때)	
				...
				%>
			<a href="./AdminMemberList.am?pageNum=<%=startPage-pageBlock%>"></a>
			<li class="page-item"><a class="page-link" href="./AdminMemberList.am?pageNum=<%=startPage-pageBlock%>"
			...
			for (int p = startPage; p<=endPage; p++) {	// 페이지의 최대 갯수를 계산해주고 그 값을 최대 페이지로 지정하여 for문을 생성	
				...
				%>
					<li class="page-item"><a href="./AdminMemberList.am?pageNum=<%=p%>" class="page-link"><%=p%></a></li>				
				<%
				}
			}
			// [다음] 버튼을 표시하기
			...
			// 현재 페이지 번호 = 첫 페이지 - 한 화면에 보여줄 페이지 수의 값을 가진 페이지로 이동
			%>
			<li class="page-item"><a class="page-link" href="./AdminMemberList.am?pageNum=<%=startPage+pageBlock%>"aria-label="Next"> 
